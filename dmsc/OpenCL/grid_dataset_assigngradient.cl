...
inline void set_edge_order
( __read_only image3d_t  func_img,
  __read_only image3d_t  flag_img,
  __global flag_t   * order_buf,
  dataset_t ds,cell_t e)
{
  if( !contains(ds.e,e) )
    return;

  flag_t fg = read_imageui(flag_img, flag_sampler, to_int4(e-ds.e.lc)).x;
  cell_t  v = flag_to_mxfct(e,fg);
  ...
  if(contains(ds.e,e0) && compare_edges(func_img,flag_img,ds,e0,e)) p  += 16;
  if(contains(ds.e,e1) && compare_edges(func_img,flag_img,ds,e1,e)) p  += 16;
  if(contains(ds.e,e2) && compare_edges(func_img,flag_img,ds,e2,e)) p  += 16;
  if(contains(ds.e,e3) && compare_edges(func_img,flag_img,ds,e3,e)) p  += 16;
  if(contains(ds.e,e4) && compare_edges(func_img,flag_img,ds,e4,e)) p  += 16;
  if(contains(ds.e,e5) && compare_edges(func_img,flag_img,ds,e5,e)) p  += 16;

  order_buf[c_to_i(ds.e,e)] = p;
}

inline void set_face_order
( __read_only image3d_t  func_img,
  __read_only image3d_t  flag_img,
  __global flag_t   * order_buf,
  dataset_t ds,cell_t f)
{
  if( !contains(ds.e,f) )
    return;

  flag_t fg = read_imageui(flag_img, flag_sampler, to_int4(f-ds.e.lc)).x;
  cell_t  e = flag_to_mxfct(f,fg);

  cell_t  d1 = swiz(to_cell(e.x&1,e.y&1,e.z&1));
  cell_t  d2 = swiz(d1);
  ...
  int p = order_buf[c_to_i(ds.e,e)]+4;

  if(contains(ds.e,f0) && compare_faces(func_img,flag_img,ds,f0,f)) p  += 4;
  if(contains(ds.e,f1) && compare_faces(func_img,flag_img,ds,f1,f)) p  += 4;
  if(contains(ds.e,f2) && compare_faces(func_img,flag_img,ds,f2,f)) p  += 4;
  if(contains(ds.e,f3) && compare_faces(func_img,flag_img,ds,f3,f)) p  += 4;

  order_buf[c_to_i(ds.e,f)] = p;
}

inline void set_cube_order
( __read_only image3d_t  func_img,
  __read_only image3d_t  flag_img,
  __global flag_t   * order_buf,
  dataset_t ds,cell_t c)
{
  if( !contains(ds.e,c) )
    return;

  flag_t fg = read_imageui(flag_img, flag_sampler, to_int4(c-ds.e.lc)).x;
  cell_t  f = flag_to_mxfct(c,fg);

  cell_t  d = to_cell((f.x+1)&1,(f.y+1)&1,(f.z+1)&1);

  cell_t c0 = f - d;
  cell_t c1 = f + d;

  int p = order_buf[c_to_i(ds.e,f)]+2;

  if(contains(ds.e,c0) && compare_cubes(func_img,flag_img,ds,c0,c)) p  += 2;
  if(contains(ds.e,c1) && compare_cubes(func_img,flag_img,ds,c1,c)) p  += 2;

  order_buf[c_to_i(ds.e,c)] = p;
}


__kernel void assign_est_order_edge
( __read_only image3d_t  func_img,
  __read_only image3d_t  flag_img,
  cell_t rct_lc,  cell_t rct_uc,
  cell_t ext_lc,  cell_t ext_uc,
  ...
  __global flag_t   * order_buf)
{
  ...
  for( int i = tid ; i < N ; i += num_thds)
  {
    ...
    set_edge_order(func_img,flag_img,order_buf,ds,v+Xdir);
    set_edge_order(func_img,flag_img,order_buf,ds,v+Ydir);
    set_edge_order(func_img,flag_img,order_buf,ds,v+Zdir);
  }
}

__kernel void assign_est_order_face
( __read_only image3d_t  func_img,
  __read_only image3d_t  flag_img,
  cell_t rct_lc,  cell_t rct_uc,
  cell_t ext_lc,  cell_t ext_uc,
  ...
  __global flag_t   * order_buf)
{
  ...
  for( int i = tid ; i < N ; i += num_thds)
  {
    ...
    set_face_order(func_img,flag_img,order_buf,ds,v+ZXdir);
  }
}

__kernel void assign_est_order_cube
( __read_only image3d_t  func_img,
  __read_only image3d_t  flag_img,
  cell_t rct_lc,  cell_t rct_uc,
  cell_t ext_lc,  cell_t ext_uc,
  ...
  __global flag_t   * order_buf)
{
  ...
  for( int i = tid ; i < N ; i += num_thds)
  {
    ...
    set_cube_order(func_img,flag_img,order_buf,ds,v+XYZdir);
  }
}

__kernel void adjust_est_order_for_pairs
( __read_only image3d_t  func_img,
  __read_only image3d_t  flag_img,
  cell_t rct_lc,  cell_t rct_uc,
  cell_t ext_lc,  cell_t ext_uc,
  ...
  __global flag_t   * order_buf)
{
  ...
  for( int i = tid ; i < N ; i += num_thds)
  {
    ...
    if(is_paired(fg))
    {
      ...
      if(cell_dim(c) < cell_dim(p))
      {
        order_buf[c_to_i(ds.e,c)] = order_buf[c_to_i(ds.e,p)]+1;
      }
    }
  }
}
